...
public class SocketIO_old {
	...
	public SocketIO_old(String server_url, String options) 
	{
		internalBrowser = new BrowserComponent();
		//internalBrowser.setDebugMode(true);
		initBrowserComponent(server_url, options);
	}
	
	
	private void initBrowserComponent(final String server_url, final String options) 
	{
		try
		{
			/*
			...
			*/
			...
			internalBrowser.addWebEventListener("onLoad", new ActionListener<ActionEvent>() {
	                public void actionPerformed(ActionEvent evt) {
	                	...
	                	internalBrowser.execute("window.socket = io('"+server_url+"'"+(options!=null?", "+options:"")+")");
	...
	public SocketIO_old connect() {
		return this.open();
	}
	
	public SocketIO_old send(final String[] args, final SuccessCallback<String> callback) {
	...
	public SocketIO_old emit(final String eventName, final String[] args, final SuccessCallback<String> callback) {
		String jscmd = "logger.log(\"emit "+eventName+"\"); socket.emit('"+eventName+"'";
		if (args!=null) {
			for(String arg : args){
				if (arg != null) {
					jscmd+=", "+arg;
	...
	public SocketIO_old on(final String eventName, final SuccessCallback<String> callback) {
		if (browserBridge.ready) {
	...
	public SocketIO_old once(final String eventName, final SuccessCallback<String> callback) {
		if (browserBridge.ready) {
	...
	public SocketIO_old compress(final boolean value) {
		if (browserBridge.ready) {
	...
	public SocketIO_old disconnect() {
		return this.close();
	}
